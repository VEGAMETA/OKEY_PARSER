...
class OKEYParser(BaseParser):
    ...
    def __init__(self, location: str, parsing_categories: list[str], proxies: list[dict[str, str]]) -> None:
        """
        Initializes asynchronous session and lock, base Okey's urls, proxies cookies and misc
        :param location:
        :param parsing_categories:
        """
        self.location = location
        self.parsing_categories = parsing_categories
        self.session = curl_cffi.requests.AsyncSession(impersonate='chrome120', verify=False)
        self.lock = asyncio.Lock()
        self.proxies_cookies = {proxy.get("https"): {} for proxy in proxies}
    ...
    async def get_response_with_proxy_cookies(
            self,
            url: str,
    ...
    async def _get_base_store_info_from_location(self) -> dict:
        ...
        """
        url = self.base_api_url + "store/0/address/find?address=" + urllib.parse.quote_plus(self.location)
    ...
    async def _get_extended_store_info_from_ffc_id(self, ffc_id: str) -> dict:
        """
    ...
    async def extend_item_data_from_html(self, html: str) -> None:
        """
    ...
    async def get_category_id(self, url: str) -> str:
        """
    ...
    async def paginate(self, formed_url: str) -> None:
        """
        Parse paginated responses
        :param formed_url: url is for api that loads each next page (the first one doesn't count)
        :return:
        """
        response = await self.get_response_with_proxy_cookies(formed_url, RequestMethod.POST)
        html = response.text
        await self.extend_item_data_from_html(html)

    async def parse_categories(self, category_url: str) -> None:
        """
        Category parser
    ...
    async def parse(self) -> None:
        ...
        parse_tasks = [self.parse_categories(category) for category in self.parsing_categories]
